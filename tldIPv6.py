...
def get_root_server_ips():
    ...
    root_ns_rr = dns.resolver.query('root-servers.net', dns.rdatatype.NS)
    ...
    for root_server_name in root_server_names:
        ipv4_rr = dns.resolver.query(root_server_name, dns.rdatatype.A)
        for rr in ipv4_rr:
            if rr.rdtype == dns.rdatatype.A:
                root_server_ips.add(rr.address)
        ipv6_rr = dns.resolver.query(root_server_name, dns.rdatatype.AAAA)
        for rr in ipv6_rr:
...
def ipv6_ns_check(domain):
    ...
    try:
        ns_rr = dns.resolver.query(domain, dns.rdatatype.NS)
    ...
    for ns_name in ns_names:
        try:
            ipv6_rr = dns.resolver.query(ns_name, dns.rdatatype.AAAA)
            for rr in ipv6_rr:
                if rr.rdtype == dns.rdatatype.AAAA:
                    return None
